...
export default function SettingsPage() {
  ...
  const fetchTenant = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to load pharmacy details');
      }
  ...
  const handleLogoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setMessage('Logo uploaded successfully. Save changes to apply it to the pharmacy profile.');
    } catch (err: unknown) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to save pharmacy details');
      }

      setMessage(result.message || 'Pharmacy details saved');
      if (result.tenant) {
        setForm({
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <form onSubmit={handleSubmit} className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div>
            <h2 className="text-xl font-semibold text-slate-950">Pharmacy details</h2>
              ...
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-4 transition hover:border-blue-300 hover:bg-white">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-slate-900">Upload a pharmacy logo</p>
            ...
            <div className="space-y-4 p-5">
              <div className="flex items-center gap-4 rounded-2xl bg-slate-50 p-4">
                ...
                <div>
                  ...
                  <p className="text-xl font-semibold text-slate-950">{form.name || 'Pharmacy name'}</p>
