...
export default function DoctorsDashboardPage() {
  ...
  const [availabilityDoctor, setAvailabilityDoctor] = useState<{ id: string; source: string } | null>(null);
  ...
  const handleDoctorSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json()).error || 'Failed to create doctor');
  ...
  const handleLinkPrescription = async (e: React.FormEvent) => {
    e.preventDefault();
    setSavingLink(true);
    setError('');
    ...
    try {
      ...
      const res = await fetch(`/api/prescriptions/${linkForm.prescriptionId}/link-sale`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        ...
        body: JSON.stringify({
          saleId: linkForm.saleId,
        }),
      });

      if (!res.ok) throw new Error((await res.json()).error || 'Failed to link sale');
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
          <div className="relative overflow-hidden bg-slate-950 px-6 py-7 text-white sm:px-8">
            <div className="absolute right-[-4rem] top-[-4rem] h-64 w-64 rounded-full bg-teal-500/20 blur-3xl" />
            <div className="absolute bottom-[-5rem] left-20 h-72 w-72 rounded-full bg-emerald-500/10 blur-3xl" />
            ...
            <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
                  Manage doctors, patients, prescriptions, uploads, and sale linkage from one clean workspace.
                </p>
              </div>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            },
            {
              ...
              value: analytics?.linkedPrescriptions || prescriptions.filter((p) => p.status === 'linked').length,
              icon: Link2,
        ...
        <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
                ...
                analyticsBars.map((item) => (
                  <div key={item.doctorId}>
                    ...
                    <div className="h-2.5 rounded-full bg-slate-100">
                      <div
                        className="h-2.5 rounded-full bg-gradient-to-r from-teal-600 to-emerald-500 transition-all"
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 rounded-2xl border border-teal-100 bg-teal-50 px-4 py-3 text-sm font-medium text-teal-800">
              Use sale linkage to connect a prescription with the final invoice.
            </div>
          </div>
        </section>
          ...
          <div className="space-y-6">
            ...
            >
              <SectionTitle
                icon={User}
                title="Add Patient"
                description="Register patient details and medical history."
              />
              ...
              <div className="mt-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    value={patientForm.historySummary}
                    onChange={(e) => setPatientForm({ ...patientForm, historySummary: e.target.value })}
                  />
                </div>
              </div>
              ...
              <div className="mt-5">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-2xl bg-violet-600 px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-violet-700 disabled:opacity-60"
                >
          ...
          <div className="space-y-6">
            {activeTab === 'prescriptions' ? (
              <>
                ...
                <form
                  onSubmit={handleLinkPrescription}
                  className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"
                >
                  <SectionTitle
                    icon={Link2}
                    title="Link Prescription to Sale"
                    description="Attach a prescription to invoice or sale record."
                  />
                  ...
                  <div className="mt-6 space-y-4">
                    ...
                    <div>
                      <label className={labelCls}>Sale ID</label>
                      <input
                        className={inputCls}
                        placeholder="Sale / Invoice ID"
                        value={linkForm.saleId}
                        onChange={(e) =>
                          setLinkForm({
                            ...linkForm,
                  ...
                  <div className="mt-5">
                    <button
                      disabled={savingLink}
                      className="inline-flex items-center gap-2 rounded-2xl bg-emerald-600 px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-emerald-700 disabled:opacity-60"
                    >
                      {savingLink ? <Loader2 className="h-4 w-4 animate-spin" /> : <Link2 className="h-4 w-4" />}
                      {savingLink ? 'Linking' : 'Link to Sale'}
                    </button>
                  </div>
                </form>
              </>
                  ...
                  {doctors.length > 0 ? (
                    doctors.map((doctor) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() => setAvailabilityDoctor({ id: doctor._id, source: 'profile' })}
                          className="ml-2 inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-slate-700 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700"
                        >
              ...
              {availabilityDoctor && (
                <AvailabilityManager doctor={availabilityDoctor} onClose={() => setAvailabilityDoctor(null)} />
              )}
              </div>
            ) : (
              <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-4 border-b border-slate-100 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
            <SectionTitle
              ...
              description="Latest prescription records and linkage status."
            />
